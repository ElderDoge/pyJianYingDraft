...
#### 添加智能调色
智能调色支持两种落法：直接挂在视频片段上，或者放在独立的调节轨上。其强度参数会按剪映草稿原始0~1浮点值直接导出，不会转换为0~100。

片段级智能调色：
```python
video_segment = draft.VideoSegment("video.mp4", trange("0s", "8s"))
video_segment.add_smart_color_adjust(0.8)
script.add_segment(video_segment)
```

调节轨级智能调色：
```python
script.add_smart_color_adjust(0.2835536858974359, trange("0s", "16.066666s"))
```

如果需要复用已有调节轨，可以显式指定轨道名：
```python
script.add_track(draft.TrackType.adjust, "global_adjust")
script.add_smart_color_adjust(0.5, trange("2s", "6s"), track_name="global_adjust", placeholder_name="调节2")
```
