...
#### 添加智能调色
智能调色支持两种落法：直接挂在视频片段上，或者放在独立的调节轨上。和普通滤镜一样，对外接口统一使用`0~100`强度；导出草稿时会自动换算成剪映需要的`0~1`原始值。

片段级智能调色：
```python
video_segment = draft.VideoSegment("video.mp4", trange("0s", "8s"))
video_segment.add_smart_color_adjust(80)
script.add_segment(video_segment)
```

调节轨级智能调色：
```python
script.add_smart_color_adjust(28.35536858974359, trange("0s", "16.066666s"))
```

如果需要复用已有调节轨，可以显式指定轨道名：
```python
script.add_track(draft.TrackType.adjust, "global_adjust")
script.add_smart_color_adjust(50, trange("2s", "6s"), track_name="global_adjust", placeholder_name="调节2")
```
